...
if __name__ == "__main__":
    ...
    for c_k_led, c_mu_led in zip(c_k, c_mu):
        ...
        grid_sizes = [80, 120, 160, 240, 320, 400, 640, 1000]
        ...
        for grid_size, num_steps, i in zip(grid_sizes, num_stepss, range(len(grid_sizes))):
            ...
            for pp_interval in pp_intervals:
                ...
                axs1[0].loglog(delta_xs[::2], l2_errors_u[::2], marker="o", markersize=8, color="black", label=f"pp int. {pp_intervals[0]}")
                axs1[0].loglog(delta_xs[1::2], l2_errors_u[1::2], marker="o", markersize=8, color="green", label=f"pp int. {pp_intervals[1]}")
                ...
                axs1[0].legend()
                axs1[1].loglog(delta_xs[::2], l2_errors_sigma[::2], marker="o", markersize=8, color="black", label=f"pp int. {pp_intervals[0]}")
                axs1[1].loglog(delta_xs[1::2], l2_errors_sigma[1::2], marker="o", markersize=8, color="green", label=f"pp int. {pp_intervals[1]}")
                axs1[1].set_xlabel("delta_x [m]")
                axs1[1].set_ylabel("error [N/m^2]")
                axs1[1].grid(True, which="both")
                axs1[1].loglog(delta_xs, C_sigma * np.array(delta_xs)**2, ":", label="Slope = 2", alpha=0.5, color="grey")
                axs1[1].legend()
                fig1.suptitle(f"Approximate L2 Error of Displacement and Stress")
                plt.savefig(f"{figures_dir}/f_0bound_dirBC_u_sig_L2_ck_ppinterval_test_pp{pp_intervals[0]}_{pp_intervals[-1]}")
                ...
                axs2[0].loglog(delta_xs[::2], linf_errors_u[::2], marker="o", markersize=8, color="black", label=f"pp int. {pp_intervals[0]}")
                axs2[0].loglog(delta_xs[1::2], linf_errors_u[1::2], marker="o", markersize=8, color="green", label=f"pp int. {pp_intervals[1]}")
                axs2[0].set_xlabel("delta_x [m]")
                axs2[0].set_ylabel("error [m]")
                axs2[0].grid(True, which="both")
                axs2[0].loglog(delta_xs, C_u * np.array(delta_xs)**2, ":", label="Slope = 2", alpha=0.5, color="grey")
                axs2[0].legend()
                axs2[1].loglog(delta_xs[::2], linf_errors_sigma[::2], marker="o", markersize=8, color="black", label=f"pp int. {pp_intervals[0]}")
                axs2[1].loglog(delta_xs[1::2], linf_errors_sigma[1::2], marker="o", markersize=8, color="green", label=f"pp int. {pp_intervals[1]}")
                axs2[1].set_xlabel("delta_x [m]")
                axs2[1].set_ylabel("error [N/m^2]")
                axs2[1].grid(True, which="both")
                axs2[1].loglog(delta_xs, C_sigma_linear * np.array(delta_xs), ":", label="Slope = 1", alpha=1, color="grey")
                axs2[1].loglog(delta_xs, C_sigma * np.array(delta_xs)**2, ":", label="Slope = 2", alpha=1, color="blue")
                axs2[1].legend()
                fig2.suptitle("Approximate LINF Error of Displacement and Stress")
                plt.savefig(f"{figures_dir}/f_0bound_dirBC_u_sig_LINF_ck_ppinterval_test_pp{pp_intervals[0]}_{pp_intervals[-1]}")
                plt.show(block=False)
