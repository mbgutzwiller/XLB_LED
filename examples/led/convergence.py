...
if __name__ == "__main__":
    ...
    grid_sizes = [40, 80, 120, 160, 240, 320]
    ...
    for c_k_led, c_mu_led in zip(c_k, c_mu):
        ...
        for grid_size, num_steps, i in zip(grid_sizes, num_stepss, range(len(grid_sizes))):
            ...
            plt.savefig(os.path.join(figures_dir, f"perBC_L2_errors_fpaper_ck_{int(np.round(c_k_led, 1)*10)}_final"), dpi=600)
            ...
            plt.savefig(os.path.join(figures_dir, f"perBC_LINF_errors_fpaper_ck_{int(np.round(c_k_led, 1)*10)}_final"), dpi=600)
            plt.show(block=False)
