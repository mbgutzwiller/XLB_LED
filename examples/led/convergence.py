...
if __name__ == "__main__":
    ...
    for c_k_led, c_mu_led in zip(c_k, c_mu):
        ...
        for grid_size, num_steps, i in zip(grid_sizes, num_stepss, range(len(grid_sizes))):
            ...
            fig1, axs1 = plt.subplots(1, 1, figsize = (12, 5), gridspec_kw={'wspace': 0.3})
            axs1.loglog(delta_xs, l2_errors_u, marker="o", markersize=8)
            axs1.set_xlabel("Grid Spacing [m]")
            axs1.set_ylabel("Rel. Error Displacement [-]")
            axs1.grid(True, which="both")
            axs1.loglog(delta_xs, C_u * np.array(delta_xs)**2, "--", label="Slope = 2", alpha=1, color="black")
            axs1.legend()
            # fig1.suptitle(r"Relative $L_{2}$ Error of Displacement and Stress", fontsize=16)
            plt.savefig(os.path.join(figures_dir, f"dirBC_L2_errors_fpaper_ck_{int(np.round(c_k_led, 1)*10)}_final_run_temp"), dpi=600)
