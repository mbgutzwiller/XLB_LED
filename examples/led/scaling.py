...
if __name__ == "__main__":
    ...
    for c_k_led, c_mu_led in zip(c_k, c_mu):
        ...
        for grid_size, num_steps, i in zip(grid_sizes, num_stepss, range(len(grid_sizes))):
            ...
            wp.build.clear_kernel_cache()
            stime_run = time.time()
            error_u, error_sigma, linf_error_u, linf_error_sigma, runtime = simulation.run(num_steps=num_steps, post_process_interval=num_steps+1)
            ...
            axs2.loglog(delta_xs, runtimes, marker="o", markersize=8)
            axs2.set_xlabel("Grid Spacing [m]")
            axs2.set_ylabel("Runtime [s]")
            axs2.grid(True, which="both")
            # axs2.loglog(delta_xs, C_u * np.array(delta_xs)**2, "--", label="Slope = 2", alpha=1, color="black")
            axs2.legend()
            # fig2.suptitle(r"Relative $L_{\infty}$ Error of Displacement and Stress", fontsize=16)
            plt.savefig(os.path.join(figures_dir, f"runtime_final"), dpi=600)
