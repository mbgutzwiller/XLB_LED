...
class ImplementationStep_LED(Enum):
    COLLISION = auto()
    STREAMING = auto()


class BoundaryCondition_LED(Operator):
    """
    Base class for boundary conditions in a LBM simulation.
    """
    ...
    def __init__(
        self,
        implementation_step: ImplementationStep_LED,
    ...
    def _construct_kernel(self, functional):
        ...
        """
        bc_helper = HelperFunctionsBC_LED(velocity_set=self.velocity_set, precision_policy=self.precision_policy, compute_backend=self.compute_backend)
                ...
                for l in range(1, 4):
                    for m in range(5):
                        if _missing_mask[l * 5 + m] == wp.uint8(1) and counter < _num_of_aux_data:
                            f_1[_opp_indices[l] * 5 + m, index[0], index[1], index[2]] = self.store_dtype(prescribed_values[counter])
                            counter += 1
